...
class SlachNormy(commands.Cog):
    ...
    async def mudar_nome(self, interaction: discord.Interaction, nome: str):
        ...
        await interaction.response.defer(ephemeral=True)  # Defer para evitar "O bot não respondeu"
        try:
            await interaction.user.edit(nick=nome)
            await interaction.followup.send(f"Seu nome foi alterado para: {nome}", 
                ephemeral=True
            )
