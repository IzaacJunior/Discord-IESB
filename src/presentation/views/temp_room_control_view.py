    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if limit < 0 or limit > 99:
                await interaction.response.send_message(
                    "❌ Limite deve estar entre 0 e 99!", ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                "❌ Por favor, digite apenas números!", ephemeral=True
            )
        except discord.Forbidden:
            await interaction.response.send_message(
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if member.id == self.owner_id:
                await interaction.response.send_message(
                    "😅 Você já está na sala!", ephemeral=True
                )
                return

            if member.bot:
                await interaction.response.send_message(
                    "🤖 Não é possível adicionar bots!", ephemeral=True
                )
                return
...
class TempRoomControlView(discord.ui.View):
    ...
    def __init__(
        self, voice_channel: VoiceChannel, owner_id: int, timeout: float | None = None
    ) -> None:
        """
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message(
                "❌ Apenas o dono da sala pode usar estes controles!", ephemeral=True
            )
            return False
        return True

    @discord.ui.button(
        label="✏️ Renomear", style=discord.ButtonStyle.primary, custom_id="rename_room"
    )
    ...
    @discord.ui.button(
        label="👥 Limite", style=discord.ButtonStyle.primary, custom_id="change_limit"
    )
    ...
    async def private_button(
        ...
        try:
            ...
            await self.voice_channel.set_permissions(
                interaction.guild.default_role, view_channel=False, connect=False
            )
            ...
            if owner:
                await self.voice_channel.set_permissions(
                    owner, view_channel=True, connect=True, manage_channels=True
                )
    ...
    async def public_button(
        ...
        try:
            ...
            await self.voice_channel.set_permissions(
                interaction.guild.default_role, view_channel=True, connect=True
            )
...
def create_temp_room_embed(
    voice_channel: VoiceChannel, owner: discord.Member
) -> discord.Embed:
    """
